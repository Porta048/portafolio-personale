...
document.addEventListener('click', (e)=>{
    ...
    if (navMenu?.classList.contains('active') && !navMenu.contains(target) && !hamburger?.contains(target)) closeMobileMenu();
});
...
let startY = 0;
document.addEventListener('touchstart', (e)=>{
    if (navMenu?.classList.contains('active')) startY = e.touches[0].clientY;
}, {
    passive: true
});
...
function initPlexusAnimation(canvas) {
    ...
    const updateSnake = ()=>{
        ...
        if (snakeTarget >= points.length) snakeTarget = 0;
        ...
        const distance = Math.sqrt(dx * dx + dy * dy);
        if (distance < 20) // Cambia target quando raggiunge il punto
...
//# sourceMappingURL=portafolio-personale-main.f66f603e.js.map
